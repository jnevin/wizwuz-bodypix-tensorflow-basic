    ...
    ***************************************************************************** */
    ...
    function toInternalResolutionPercentage(internalResolution) {
        if (typeof internalResolution === 'string') {
            ...
            tf.util.assert(typeof internalResolution === 'number' && internalResolution < 1 &&
                internalResolution > 0, function () {
                return "inputResolution must be a string or number between 0 and 1, but " +
    ...
    function validateMultiPersonInstanceInferenceConfig(config) {
        var maxDetections = config.maxDetections, scoreThreshold = config.scoreThreshold, nmsRadius = config.nmsRadius, minKeypointScore = config.minKeypointScore, refineSteps = config.refineSteps;
        if (maxDetections <= 0) {
